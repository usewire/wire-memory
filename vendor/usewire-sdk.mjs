...
// node_modules/@usewire/sdk/dist/crypto.js
var JWT_LIFETIME_SECONDS = 60;
...
// node_modules/@usewire/sdk/dist/types.js
var WireSdkError = class extends Error {
  code;
  status;
...
// node_modules/@usewire/sdk/dist/client.js
var API_BASE = "https://app.usewire.io";
var DEFAULT_CONSENT_PATH = "/sdk/connect";
...
var WireClient = class {
  ...
  async connect(options = {}) {
    ...
    if (isBootstrap)
      connectBody.public_key = deviceKey.publicKey;
    ...
    const connectData = await unwrap(connectRes);
  ...
  async getStatus(apiKey) {
    if (!apiKey)
      throw new WireSdkError("NOT_CONNECTED", "apiKey is required");
    ...
    return {
      container: {
        ...
        ephemeralExpiresAt: data.container.ephemeral_expires_at ? new Date(data.container.ephemeral_expires_at) : null
      },
      connection: {
  ...
  async pollForReady(nonce) {
    const start = Date.now();
    while (Date.now() - start < POLL_TIMEOUT_MS) {
      try {
        const res = await fetch(`${API_BASE}/api/v1/sdk/poll?nonce=${encodeURIComponent(nonce)}`);
        const data = await res.json();
        if (data.status === "ready")
          return data;
        if (data.status === "expired") {
          throw new WireSdkError("NONCE_EXPIRED", "Connection session expired. Please try again.");
        }
      } catch (err) {
        if (err instanceof WireSdkError)
          throw err;
      }
      await new Promise((r) => setTimeout(r, POLL_INTERVAL_MS));
    }
    throw new WireSdkError("POLL_TIMEOUT", "Connection timed out after 5 minutes. Please try again.");
  }
};
...
async function defaultUserPrompt(code, url) {
  console.log(`
Your code: ${code}`);
...
`);
  ...
  try {
    ...
    const cmd = proc.platform === "darwin" ? "open" : proc.platform === "win32" ? "start" : "xdg-open";
...
async function unwrap(res) {
  ...
  if (!res.ok || !json.success || !json.data) {
    ...
    const message2 = json.error?.message ?? `Request failed: ${res.status}`;
    throw new WireSdkError(code, message2, res.status);
  }
  return json.data;
}
export {
